...
function switchTab(tab) {
  const views = ['dashboard', 'candle', 'ai', 'backtest', 'research'];
  views.forEach(v => {
...
async function runResearch() {
  const tickersInput = $('research-tickers-input').value.trim();
  if (!tickersInput) {
    showToast('Vui lòng nhập danh sách mã cổ phiếu!', 'error');
    return;
  }

  const btn = $('btn-run-research');
  const loading = $('research-loading');
  const resultsContainer = $('research-results-container');
  const eventsCard = $('research-events-card');
  ...
  try {
    const response = await fetch(`${API_BASE}/api/research?tickers=${encodeURIComponent(tickersInput)}`);
    if (!response.ok) {
      const err = await response.json();
      throw new Error(err.detail || 'Lỗi không xác định khi chạy nghiên cứu');
    }

    const data = await response.json();
    
    // Render kết quả
    renderResearchWeights(data.optimal_weights);
    renderResearchHeatmap(data.pattern_stats);
    renderResearchEvents(data.ticker_results);

    // Show results
    loading.style.display = 'none';
    resultsContainer.style.display = 'block';
    eventsCard.style.display = 'block';
    showToast(`✅ Đã nghiên cứu xong ${data.total_events_analyzed} sự kiện!`, 'success');
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-play"></i> Chạy nghiên cứu';
  }
}

function renderResearchWeights(weights) {
  const grid = $('research-weights-grid');
  grid.innerHTML = '';

  if (!weights || Object.keys(weights).length === 0) {
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; padding:20px; color:var(--text-muted);">Không có dữ liệu trọng số. Chạy nghiên cứu trước.</div>';
    return;
  }

  // Sắp xếp trọng số giảm dần
  const sorted = Object.entries(weights).sort((a, b) => b[1].weight - a[1].weight);

  sorted.forEach(([key, val]) => {
    const card = document.createElement('div');
    card.className = 'weight-card';
    card.style.setProperty('--progress', `${val.weight}%`);

    card.innerHTML = `
      <div class="weight-title" title="${val.name}">${val.name}</div>
      <div class="weight-value-row">
        <span class="weight-val">${val.weight}%</span>
        <span class="weight-corr">Tương quan: ${val.correlation}%</span>
      </div>
    `;
...
function renderResearchHeatmap(stats) {
  const tbody = $('research-heatmap-tbody');
  tbody.innerHTML = '';

  if (!stats || Object.keys(stats).length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:var(--text-muted);">Không có dữ liệu thống kê. Chạy nghiên cứu trước.</td></tr>';
    return;
  }

  // Map khóa chỉ báo từ backend sang nhãn tiếng Việt
  const metricLabels = {
    'macd_hist_dir_up': 'Độ dốc MACD Histogram tăng',
    'macd_hist_red_light': 'MACD Histogram đỏ nhạt (thu nhỏ âm)',
    'macd_golden_cross': 'MACD Golden Cross gần đây (3 phiên)',
    'macd_vs_signal_above': 'MACD Line nằm trên Signal Line',
    'price_above_ma10': 'Giá đóng cửa nằm trên MA10',
    'price_above_ma20': 'Giá đóng cửa nằm trên MA20',
    'price_above_ma50': 'Giá đóng cửa nằm trên MA50',
    'bb_below_02': 'Bollinger Bands %B < 0.2 (Giá chạm/dưới dải dưới)',
    'bb_below_05': 'Bollinger Bands %B < 0.5 (Nửa dưới dải)',
    'vol_above_1_2x': 'Khối lượng giao dịch đột biến > 1.2x MA20',
    'vol_above_1_5x': 'Khối lượng giao dịch đột biến > 1.5x MA20',
    'has_bullish_candle': 'Xuất hiện mô hình nến đảo chiều tăng (Bullish)',
    'has_bearish_candle': 'Xuất hiện mô hình nến đảo chiều giảm (Bearish)',
    'obv_slope_positive': 'Độ dốc OBV dương (Dòng tiền tích lũy)',
    'near_support': 'Giá đóng cửa gần hỗ trợ 60 phiên (< 2%)',
    'near_resistance': 'Giá đóng cửa gần kháng cự 60 phiên (< 2%)'
  };

  // Trả về mã màu nền cho heatmap dựa trên tỷ lệ % (xanh cho tăng, đỏ cho giảm)
  function getHeatmapBg(pct, isBullishEvent) {
    const alpha = (pct / 100) * 0.45; // opacity tối đa 0.45
    if (isBullishEvent) {
      return `rgba(16, 185, 129, ${alpha})`; // xanh
    } else {
      return `rgba(239, 68, 68, ${alpha})`; // đỏ
    }
  }

  Object.entries(metricLabels).forEach(([key, name]) => {
    const tr = document.createElement('tr');

    const surgeUpPct = stats['surge_up']?.metrics?.[key]?.pct ?? 0;
    const revUpPct = stats['reversal_up']?.metrics?.[key]?.pct ?? 0;
    const surgeDownPct = stats['surge_down']?.metrics?.[key]?.pct ?? 0;
    const revDownPct = stats['reversal_down']?.metrics?.[key]?.pct ?? 0;
...
function renderResearchEvents(results) {
  const tbody = $('research-events-tbody');
  tbody.innerHTML = '';

  if (!results || Object.keys(results).length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding:20px; color:var(--text-muted);">Không có sự kiện.</td></tr>';
    return;
  }

  // Gom tất cả sự kiện và sắp xếp
  let allEvents = [];
  ...
  // Sắp xếp ngày kích hoạt giảm dần (ngày gần nhất trước)
  allEvents.sort((a, b) => {
    const dateA = a.trigger_date.split('/').reverse().join('-');
    const dateB = b.trigger_date.split('/').reverse().join('-');
    return dateB.localeCompare(dateA);
  });

  // Hiển thị tối đa 20 sự kiện gần nhất
  const limitEvents = allEvents.slice(0, 20);

  if (limitEvents.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding:20px; color:var(--text-muted);">Không tìm thấy sự kiện biến động nào.</td></tr>';
    return;
  }

  const eventTypes = {
    'surge_up': { label: 'Tăng mạnh', badgeClass: 'text-green' },
    'reversal_up': { label: 'Đảo chiều tăng', badgeClass: 'text-green font-bold' },
    'surge_down': { label: 'Giảm mạnh', badgeClass: 'text-red' },
    'reversal_down': { label: 'Đảo chiều giảm', badgeClass: 'text-red font-bold' }
  };

  limitEvents.forEach(e => {
    const tr = document.createElement('tr');
    const et = eventTypes[e.type] || { label: e.type, badgeClass: '' };
    
    const changeSign = e.price_change_pct > 0 ? '+' : '';
    const changeColor = e.price_change_pct > 0 ? 'text-green' : 'text-red';
    const candleText = e.context.candle_pattern || 'Không có';
